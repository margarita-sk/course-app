...
public class StudentServiceImpl implements StudentService {
    ...
    public StudentServiceImpl(StudentRepository studentRepository, CourseRepository courseRepository, Client client) {
    ...
    public Collection<StudentOktaDto> receiveAll() {
        var studentsDto = receiveStudentsBothDbOkta().stream().map(user -> {
            ...
            return new StudentOktaDto(profile.getEmail(), profile.getFirstName(), profile.getLastName(), faculty, courses);
    ...
    public StudentOktaDto findStudentByEmail(String email) {
        ...
        var studentFromOkta = receiveStudentsBothDbOkta().stream().filter(user -> email.equals(user.getProfile().getEmail())).findFirst().orElseThrow(NoSuchStudentException::new);
        var studentOktaDto = new StudentOktaDto(studentFromOkta.getProfile().getEmail(), studentFromOkta.getProfile().getFirstName(), studentFromOkta.getProfile().getLastName(), studentFromDb.getFaculty(), studentFromDb.getCourses());
    ...
    public void deleteStudent(String email) {
        ...
        var user = client.listUsers().stream().filter(searchedUser -> searchedUser.getProfile().getEmail().equals(email)).findFirst().orElseThrow(NoSuchStudentException::new);
        user.deactivate();
        user.delete();
    }
    ...
    private Student receiveStudentByEmail(String email) {
        var student = studentRepository.findById(email).orElseThrow(() -> {
    ...
    private List<User> receiveStudentsBothDbOkta() {
        ...
        var studentsFromDB = StreamSupport.stream(studentRepository.findAll().spliterator(), false).collect(Collectors.toList());
        var students = studentsFromOktaStream.filter(user ->
                studentsFromDB.stream().anyMatch(student -> student.getEmail().equals(user.getProfile().getEmail()))).collect(Collectors.toList());
        return students;
    }
