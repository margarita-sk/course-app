...
public class TaskServiceImpl implements TaskService {
    ...
    public TaskDto receiveTaskById(int courseId, int taskId) {
        ...
        var searchedTask =
                course.getTasks().stream().filter(task -> task.getId() == taskId).findFirst().orElseThrow(() -> {
    ...
    public Collection<TaskDtoShort> receiveAllTasksByCourse(int courseId) {
        ...
        var tasksDto = tasks.stream().map(task -> new TaskDtoShort(task.getName())).collect(Collectors.toList());
        return tasksDto;
    }
